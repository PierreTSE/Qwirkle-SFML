...
Tile::Tile(uint8_t shapeID, TileColor color) : shapeID{shapeID}, color{color} {
    assert(shapeID <= 6);
...
Tile::Tile(TileData const& rhs) : Tile{rhs.shapeID, rhs.color} {}
    ...
    if (colorblind && std::get<bool>(colorblind.value())) {
        ...
        switch (type) {
            ...
            case Pink:
                return {136, 204, 238};
            default:
                throw std::runtime_error("type2color default case");
        }
    }
}
